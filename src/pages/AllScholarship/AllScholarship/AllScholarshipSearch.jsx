...
const AllScholarshipSearch = () => {
  ...
  const callData = async (text) => {
    axios.get(`${import.meta.env.VITE_VERCEL_API}/listSearch?input=${text}`)
      .then(function (response) {
        // handle success
  ...
  const searchByTyping = async (e) => {
    const searchTyping = e.target.value;
    await callData(searchTyping);
    setSearchText(searchTyping);
  }
  ...
  useEffect(() => {
    document.getElementById("myDropdown").size = 10;
  ...
  return (
    <div>
      ...
      <h3 className="bg-base-300 w-full p-5 md:p-8 text-2xl md:text-5xl font-bold text-center rounded-3xl my-5">All Scholarships Search </h3>
      {/* ----- filter start ----- */}
      ...
      <div className="w-1/2 mx-auto relative ">
        <label className="flex items-center input input-bordered gap-3">
          <FiSearch />
          <input type="text" onFocus={inputFocus} onBlurCapture={inputBlur} onChangeCapture={searchByTyping} name='Search' placeholder="Search" className="w-full " required />
        </label>
        <select
          id="myDropdown"
          className={`select absolute select-bordered w-full max-h-80 z-50 ${isOpen ? 'visible' : 'invisible'}`}
          style={{ height: `${(data.length + 1) * 40}px` }}
          onChange={handleSelectItem}
        >
          <option disabled value="nothingFound">
            {data.length > 0 ? 'Select your choice.' : 'Nothing Found'}
          </option>
          {data?.map((item, idx) => (
            <option
              className="btn"
      ...
      <div className="mx-auto w-1/2 mt-5 font-semibold text-xl">
        <p>{searchText}</p>
      </div> */}
      {/* fixed search result start */}
      {/* <div className="mx-auto w-1/2 mt-5 font-semibold text-xl">
        <div className="flex flex-col gap-1" name="selectData" id="">
          {
            data.map((item, idx) => <span onClick={handleSelectItem} id={item._id}
              className="btn"
              key={idx} >{item.scholarshipName}</span>)
          }
        </div>
      </div> */}
      {/* fixed search result end */}
      {/* <div className="mx-auto w-1/2 mt-5 font-semibold text-xl">
        <div className="flex flex-col gap-1" name="selectData" id="">
          {
            selectItems.map((item, idx) => <span className="btn"
              key={idx} >{item.scholarshipName}</span>)
          }
        </div>
      </div>
      <div>
        {
          selectItems.map((item, idx) => <p key={idx}>{item.scholarshipName}</p>)
        }
      </div> */}
      {/* ----------------- Search field end -------------------- */}
...
export default AllScholarshipSearch;